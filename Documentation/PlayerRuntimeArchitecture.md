...
├── HitStopController
└── SquadController
    ├── 활성 캐릭터의 AgentActionController로 입력 전달
...
- `PlayerInputRouter`: 입력을 현재 `IAgentInputTarget`으로 전달한다.
...
캐릭터 프리팹에 붙는 파사드다. 해당 프리팹의 `AgentActionController`와
...
6. 입력 타깃을 새 `AgentActionController`로 변경한다.
...
1. 캐릭터 루트에 `AgentMotor`, `AgentActionController`와 필수 의존 컴포넌트를 구성한다.
2. 루트에 `AgentRoot`를 추가한다.
