...
{
    // 특수스킬 트리거 (push) — 콤보 링크(Attack=Special)가 받지 못한 경우의 전역 폴백.
    // 콤보 중에는 각 공격 섹션의 Special 링크가 윈도우 안에서 입력을 소비해 진입하고(우선),
    // 걷기/Idle처럼 링크가 없는 상태에서는 이 트리거가 입력을 받아 특수 config로 강제 진입한다.
    ...
    public class SpecialTrigger
    {
        ...
        public SpecialTrigger(PlayerStateMachine machine, ConfigState state, ConfigRegistry registry,
        ...
        public void Trigger()
        {
            ...
            if (cfg == null)
            {
                Debug.LogWarning($"[Special] '{_section}' 섹션을 가진 config가 없음 — PlayerStateMachine 'Configs' 리스트와 섹션 이름 확인", _machine);
                return;
            }
