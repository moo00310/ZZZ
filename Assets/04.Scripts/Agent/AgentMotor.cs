...
namespace ZZZ.Agent
{
    ...
    [System.Flags]
    [MovedFrom(true, "ZZZ.Player", "Assembly-CSharp", "PlayerMotorFlags")]
    public enum AgentMotorFlags
    {
        None           = 0,
        CodeMovement   = 1 << 0,
        RootMotion     = 1 << 1,
    ...
    [MovedFrom(true, "ZZZ.Player", "Assembly-CSharp", "PlayerMotor")]
    public class AgentMotor : MonoBehaviour, ICharacterMover
    {
        [Header("Locomotion")]
        ...
        public AgentMotorFlags CurrentFlags
        {
            get
            {
                AgentMotorFlags flags = AgentMotorFlags.None;
                if (UseCodeMovement) flags |= AgentMotorFlags.CodeMovement;
                if (IsRootMotionActive) flags |= AgentMotorFlags.RootMotion;
                if (!AllowRotation) flags |= AgentMotorFlags.RotationLocked;
                if (WarpWindowActive) flags |= AgentMotorFlags.WarpActive;
                return flags;
            }
        }
