...
namespace ZZZ.Agent
{
    public interface IAgentInputTarget
    {
    ...
    // 얇은 코디네이터 — config 러너(CharacterActionRunner)를 소유·구동하고, 협력 객체(입력 버퍼/트리거)를 조립한다.
    ...
    [RequireComponent(typeof(AgentResources))]
    [MovedFrom(true, "ZZZ.Player.StateMachine", "Assembly-CSharp", "PlayerActionController")]
    public class AgentActionController : MonoBehaviour, ICharacterActionSignals, ILiveMonitor,
        IInputMonitor, IAgentInputTarget, IHittable, IHitSource,
        IReactionTargetProvider, IParryWarningReceiver
    {
        ...
        private CharacterActionRunner        _runner;   // 단일 config 러너 — 전이는 config가 관리
        ...
        // ── 입력 버퍼 facade (CharacterActionRunner/HUD/에디터 툴이 사용) ───────
        ...
        private void Awake()
        {
            ...
            var resources  = GetComponent<AgentResources>();
            var sensor     = GetComponent<ZZZ.Combat.EnemySensor>();

            var condCtx = new AgentConditionContext(_motor, transform, this);
            var cfgCtx  = new CharacterActionContext
            {
                Mover      = _motor,
                Animator   = animator,
            ...
            _dodge.Init(this, _runner, registry, _input, resources);
            _parry.Init(this, _runner, registry, _input);
            _attackNormalEnhance.Init(this, _runner, registry, _input, sensor);
            _hit.Init(this, _runner, registry, _parry.Prefix);   // 쳐냄 섹션 접두어는 ParryTrigger에서 단일 정의
        }
