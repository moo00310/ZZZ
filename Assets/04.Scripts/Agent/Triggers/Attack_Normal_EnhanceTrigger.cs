...
namespace ZZZ.Agent
{
    ...
    // 그래서 AgentActionController는 _runner.Update(콤보 링크 평가) 이후 입력이 남아 있을 때만 이걸 부른다.
    //
    ...
    public class Attack_Normal_EnhanceTrigger
    {
        ...
        public void Trigger()
        {
            ...
            if (cfg == null)
            {
                Debug.LogWarning($"[Attack_Normal_Enhance] '{section}' 섹션을 가진 config가 없음 — AgentActionController 'Configs' 리스트와 섹션 이름 확인", _controller);
                return;
            }

            // 재진입 가드 — 이미 같은 스킬 시전 중이고 진행도가 낮으면 무시(연타로 무한 갱신 방지)
            if (_runner.CurrentConfig == cfg && _runner.ActiveSection == section
                && _runner.CurrentNormalizedTime < _reinterrupt)
            {
                _input.Consume();
                return;
            }
