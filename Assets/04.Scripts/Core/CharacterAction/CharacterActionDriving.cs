...
namespace ZZZ
{
    // CharacterActionRunner(애니메이션 config 러너)가 캐릭터에 요구하는 추상 표면들.
    // 플레이어(AgentMotor/CharacterAnimatorBridge/AgentActionController)와 몬스터가 각자 구현해
    // 같은 CharacterActionRunner 엔진을 공유한다. CharacterActionRunner는 이 인터페이스들에만 의존(구상 타입 비의존).

    // 이동/회전/루트모션/타겟 표면 — CharacterActionRunner.PlayActive 및 매 프레임 윈도우 갱신이 호출.
    ...
    // CharacterActionRunner가 접근하는 공유 컴포넌트 묶음 — 구상 데이터 홀더.
    // 인터페이스가 아니라 구상 클래스다: 다형성이 필요한 건 Mover/Animator 둘뿐이고(아래 필드 타입),
    // '묶음' 자체는 캐릭터마다 바꿔치기할 일이 없다. 플레이어/몬스터가 각자 채워 CharacterActionRunner에 넘긴다.
    public sealed class CharacterActionContext
    {
        public ICharacterMover    Mover      { get; set; }
        public IAnimatorBridge Animator   { get; set; }

        public Transform       Transform  { get; set; }
    }

    // CharacterActionRunner가 머신에 보내는 신호 — 무적/패링(섹션 진입 시 리셋, 모듈이 윈도우 동안 재설정),
    // 입력 버퍼 소비(몬스터는 no-op).
    public interface ICharacterActionSignals
    {
