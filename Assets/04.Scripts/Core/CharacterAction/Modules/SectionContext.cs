...
namespace ZZZ
{
    // 섹션 모듈이 받는 런타임 핸들 묶음. CharacterActionRunner가 1개 만들어 재사용한다.
    public class SectionContext
    {
        public CharacterActionContext  Ctx;       // 공유 컴포넌트 (모듈이 현재 미사용 — 추후 이전용)
        public ICharacterActionSignals Machine;   // 무적/패링 신호 (i-frame/parry 모듈이 사용)
